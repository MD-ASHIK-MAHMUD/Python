...
class  TEACHER:
    def __init__(self,root):
       ...
       #TITLE
        title=Label(self.root, text="SCHOOL MANAGEMENT SYSTEM --- TEACHER", font=("times new roman",40,"bold"), bg="orange").pack(side=TOP, fill=X)
        
        #FROM 

        From=Frame(self.root, bd=5, relief=RIDGE, bg="salmon").place(x=20, y=100, width=500, height=580)

        From_title=Label(From, text="TEACHER FROM: ", bg="salmon", fg="green", font=("times new roman",20,"bold")).place(x=30, y=110)

        slno=Label(From, text="SLNO NUMBER: ", bg="salmon", fg="black", font=("times new roman",18,"bold")).place(x=30, y=150, width=200, height=20)
        txt_slno=Entry(From, font=("times new roman",15,"bold"), bd=5, textvariable=self.SLNO).place(x=250, y=150, width=200, height=30)

        Id=Label(From, text="ID NUMBER: ", bg="salmon", fg="black", font=("times new roman",18,"bold")).place(x=30, y=200, width=200, height=20)
        txt_Id=Entry(From, font=("times new roman",15,"bold"), bd=5, textvariable=self.ID).place(x=250, y=200, width=200, height=30)
        ...
        qinfo=Label(From, text="QUILIFICATION INFO: ", bg="salmon", fg="black", font=("times new roman",12,"bold")).place(x=30, y=350, width=200, height=20)
        txt_qinfo=Entry(From, font=("times new roman",15,"bold"), bd=5, textvariable=self.QINFO).place(x=250, y=350, width=200, height=30)
        
        dept=Label(From, text="DEPERTMENT: ", bg="salmon", fg="black", font=("times new roman",18,"bold")).place(x=30, y=400, width=200, height=20)
        txt_dept=Entry(From, font=("times new roman",15,"bold"), bd=5, textvariable=self.DEPT).place(x=250, y=400, width=200, height=30)
        ...
        self.teacher_table=ttk.Treeview(table,column=("SLNO", "ID", "NAME", "EMAIL", "QINFO", "DEPT"),xscrollcommand=scrolly,yscrollcommand=scrollx)
    ...
    def add_data(self):
        if self.ID.get()=="" or self.NAME.get()=="":
            ...
            cur.execute("insert into teacher  values (%s,%s,%s,%s,%s,%s)", (self.SLNO.get(),self.ID.get(),self.NAME.get(),self.EMAIL.get(),self.QINFO.get(),self.DEPT.get()))
    ...
    def update_data(self):
        ...
        cur.execute("update teacher set ID=%s,NAME=%s,EMAIL=%s,QINFO=%s,DEPT=%s where SL_NO=%s", (
                                                                                                      self.ID.get(),
                                                                                                      self.NAME.get(),
                                                                                                      self.EMAIL.get(),
                                                                                                      self.QINFO.get(),
    ...
    def delete_data(self):
        ...
        cur.execute("delete from teacher where ID=%s",self.ID.get())
